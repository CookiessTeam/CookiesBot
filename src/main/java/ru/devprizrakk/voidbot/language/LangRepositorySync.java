...
public class LangRepositorySync {
    ...
    private static final String GITHUB_ZIP = "https://github.com/voidforge-community/language/archive/refs/heads/main.zip";
    ...
    public static void sync() {
        try {
            ...
            // Если папка отсутствует - скачиваем
            if (!Files.exists(langDir)) {
                Logger.getLogger().log(LogType.INFO, "loader", "Папка локализаций отсутствует - скачиваю...");
                forceSync();
                return;
            }
            ...
            if (!checkIfUpdated()) {
                Logger.getLogger().log(LogType.INFO, "loader", "Локализации актуальны, скачивание не требуется.");
                return;
            }

            Logger.getLogger().log(LogType.INFO, "loader", "Найдены изменения, скачиваю локализации из GitHub...");
            forceSync();
        } catch ( Exception e ) {
            Logger.getLogger().log(LogType.ERROR, "loader", "Ошибка загрузки локализаций из GitHub", e);
        }
    }
    ...
    private static void forceSync() throws Exception {
        ...
        Logger.getLogger().log(LogType.INFO, "loader", "Локализации успешно обновлены.");
    }
    ...
    private static boolean checkIfUpdated() {
        try {
            ...
            Files.writeString(ETAG_FILE, remoteEtag, StandardOpenOption.CREATE, StandardOpenOption.TRUNCATE_EXISTING);
            return true;
        } catch ( Exception e ) {
