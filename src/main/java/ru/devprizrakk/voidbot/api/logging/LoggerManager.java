...
import ru.devprizrakk.voidbot.api.utils.ColorConsole;
...
public class LoggerManager {
    ...
    public void log(LogType type, String category, String message, Throwable e) {
        switch (type) {
            case INFO -> info(category, message);
            case DEBUG -> debug(category, message);
            case WARN -> warn(category, message);
    ...
    private void debug(String category, String message) {
        if (true /*TODO: Реализовать Логер уровни уровни */) print("DEBUG", category, ColorConsole.ANSI_BLUE, message);
    }
    ...
    private void error(String category, String message, Throwable e) {
        error(category, message);
        if (e != null) {
            print("ERROR", category, ColorConsole.ANSI_RED, e.toString());
            for (StackTraceElement element : e.getStackTrace()) {
